   ...
   <div class="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800">
      ...
      <li>
          <a wire:navigate href="{{ route('admin.dashboard') }}" 
            ...
            {{ request()->routeIs('admin.dashboard') ? 'bg-gray-200 dark:bg-gray-800 text-indigo-600' : '' }}">
            <svg class="w-5 h-5 transition duration-75 
                {{ request()->routeIs('admin.dashboard') ? 'text-indigo-600' : 'text-gray-500 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white' }}" 
        ...
        <li>
          <a wire:navigate href="{{ route('admin.createSubject') }}" 
            class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group
            {{ request()->routeIs('admin.createSubject') ? 'bg-gray-200 dark:bg-gray-800 text-indigo-600' : '' }}">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
            </svg>

            <span class="ms-3">Create Subject</span>
          </a>
        </li>
      </ul>
   </div>
